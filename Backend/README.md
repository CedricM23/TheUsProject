# Final Capstone Java Project Starter
...
| --------- | ----------- |
| `data.sql` | This script populates the database with any static setup data or test/demo data. The project team should modify this script.<br />Note the default application users created in this script. |
...
Note: Spring Boot has been configured to run on port `9000` for this project instead of the default `8080`.
...
Any controller that'll be accessed from a client like the React Starter application needs the `@CrossOrigin` annotation. This tells the browser that you're allowing the client application to access this resource:
...
`AuthenticationController.java` handles the `/login` and `/register` routes and works with the React starter as is. If you need to modify the user registration form, start here.

The authentication controller uses the `JdbcUserDao` to read and write data from the users table.

### User controller

`UserController.java` has two endpoints `/users` and `/users/{id}` to retrieve all users or one specific user. The React starter uses the latter endpoint to validate the user token when the user returns to the application after previously has logged in.
